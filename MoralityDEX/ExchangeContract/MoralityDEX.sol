...
contract MoralityDEX is Breaker, ReentrancyGuard, StringUtil {
    ...
    // This updates existing token only - fails if doesn't exist
    function updateToken(string calldata symbol, uint256 rate, address tokenAddress) external onlyOwner {
        // Check doesn't already exist
        require(_tokens[symbol].exists == true);
        _updateOrUpdateToken(symbol, rate, tokenAddress);
    }
    
    // This adds new token only if it doesnt exist already
    function addToken(string calldata symbol, uint256 rate, address tokenAddress) external onlyOwner {
        // Check already exists
        require(_tokens[symbol].exists == false);
        _updateOrUpdateToken(symbol, rate, tokenAddress);
    }
    ...
    // This is to add or update an existing token rate or address
    function _updateOrUpdateToken(string memory symbol, uint256 rate, address tokenAddress) internal onlyOwner {
        // Basic checks 
